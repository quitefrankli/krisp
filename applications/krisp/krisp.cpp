...
public:
	...
	void on_begin(GameEngine& engine) override
	{
		...
		const auto& player_transform =
			engine.get_ecs().get_transformation(spawned_player.get_id());
		camera.look_at(player_transform.get_position() + definition.camera_focus_offset,
			player_transform.get_position() + glm::vec3(0.0f, 2.0f, -5.0f));
