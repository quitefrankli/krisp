...
protected:
	...
	{
		const glm::vec3 r = ecs->get_position(obj2.get_id()) - ecs->get_position(obj1.get_id());
	...
	{
		const glm::vec3 r = ecs->get_position(obj.get_id()) - center;
...
TEST_F(OrbitalPhysicsTest, gravitational_force_magnitude)
{
	...
	ecs->add_object(obj1);
	ecs->add_object(obj2);
	ecs->set_position(obj1.get_id(), glm::vec3(0.0f, 0.0f, 0.0f));
	ecs->set_position(obj2.get_id(), glm::vec3(10.0f, 0.0f, 0.0f));
	...
	ASSERT_NEAR(glm::length(ecs->get_position(obj2.get_id()) - p2.position), expected_force * 0.01f * 0.01f / (2.0f * p2.mass), TOLERANCE);
...
TEST_F(OrbitalPhysicsTest, force_derived_acceleration_is_persisted)
{
	...
	ecs->set_position(first.get_id(), glm::vec3(0.0f));
	ecs->set_position(second.get_id(), glm::vec3(10.0f, 0.0f, 0.0f));
...
TEST_F(OrbitalPhysicsTest, circular_orbit_stability)
{
	...
	ecs->set_position(star.get_id(), glm::vec3(0.0f));
	ecs->set_position(planet.get_id(), glm::vec3(orbit_radius, 0.0f, 0.0f));
	...
	const float final_distance = glm::length(
		ecs->get_position(planet.get_id()) - ecs->get_position(star.get_id()));
...
TEST_F(OrbitalPhysicsTest, angular_momentum_conservation)
{
	...
	const glm::vec3 initial_L = calculate_angular_momentum(
		planet, planet_phys, ecs->get_position(star.get_id()));
	...
	const glm::vec3 final_L = calculate_angular_momentum(
		planet, *final_planet_phys_ptr, ecs->get_position(star.get_id()));
