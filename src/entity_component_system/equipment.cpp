...
bool EquipmentSystem::equip(const Entity wearer, const Entity item, const EquipmentDefinition& definition)
{
	if (!valid_slot(static_cast<std::uint64_t>(definition.slot)))
		return false;
	if (!get_ecs().attach_entity_to_bone(item, wearer, definition.attachment_bone, definition.grip_transform))
		return false;
	...
	destination = EquippedItem{ item, definition };
...
void EquipmentSystem::deserialize(const Deserializer& in)
{
	...
	for (size_t index = 0; index < entries.size(); ++index)
	{
		...
		destination = EquippedItem{ item, { slot, entry.read<std::string>("attachment_bone"),
	...
	try {
		for (const auto& [wearer, slots] : restored)
			for (const auto& equipped : slots)
				if (equipped) {
					if (!get_ecs().attach_entity_to_bone(equipped->item, wearer,
		...
		for (const auto& [wearer, slots] : equipment)
			for (const auto& equipped : slots)
				if (equipped)
					get_ecs().attach_entity_to_bone(equipped->item, wearer,
						equipped->definition.attachment_bone, equipped->definition.grip_transform);
		throw;
	}
	for (const auto& [item, _] : item_locations)
