                                   ...
                                   const glm::mat4& transform)
{
	...
	if (textures.size() != object.get_shapes().size())
	{
	...
	{
		Material mat;
		mat.texture = fetch_texture(*textures_it++);
		shape->set_material(mat);
	}

	fmt::print("ResourceLoader::load_object: loaded {}, shapes:={}, vertices:={}, indices:={}, texures:={}\n",
		mesh, object.get_shapes().size(), object.get_num_unique_vertices(), object.get_num_vertex_indices(), textures.size());
...
{
	Object object;
	const auto vertex_loader = [&color](SDS::TexVertex& new_vertex, tinyobj::attrib_t& attrib, tinyobj::index_t& index) {
		memcpy(&new_vertex.pos, &attrib.vertices[3 * index.vertex_index], sizeof(new_vertex.pos));
	};
	...
	fmt::print("ResourceLoader::load_object: loaded {}, shapes:={}, vertices:={}, indices:={}\n",
		mesh, object.get_shapes().size(), object.get_num_unique_vertices(), object.get_num_vertex_indices());
	...
	const glm::mat4& transform)
{
	...
	object.get_shapes().reserve(shapes.size());
	for (auto& shape : shapes)
	{
		std::unordered_map<SDS::TexVertex, uint32_t> unique_vertices;
		
		TexShape new_shape;
		new_shape.get_indices().reserve(shape.mesh.indices.size());
		new_shape.get_vertices().reserve(attrib.vertices.size()/3); // attrib uses float to store x/y/z
		
		for (tinyobj::index_t& index : shape.mesh.indices)
		{
			std::pair<SDS::TexVertex, uint32_t> new_pair{};
			new_pair.second = new_shape.get_vertices().size();

			SDS::TexVertex& new_vertex = new_pair.first;
			...
			{
				new_shape.get_indices().emplace_back(new_shape.get_vertices().size());
				new_shape.get_vertices().push_back(std::move(new_vertex));
			} else // if already occupied, set index to the index in which said vertex occupies
			{
				new_shape.get_indices().emplace_back(unique_vertex_element.first->second);
			}
		}
		...
		object.get_shapes().push_back(std::make_unique<TexShape>(std::move(new_shape)));
	}
...
// 	objects.reserve(object.get_shapes().size());
// 	auto textures_it = textures.begin();
// 	for (auto& shape : object.get_shapes())
// 	{
// 		Material mat;
...
// 		AABB aabb = shape->calculate_bounding_box();
// 		switch (setting)
// 		{
