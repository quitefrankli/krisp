...
{
	vkDestroyCommandPool(GraphicsBufferManager<GraphicsEngineT>::get_logical_device(), command_pool, nullptr);
}

template<typename GraphicsEngineT>
void GraphicsResourceManager<GraphicsEngineT>::create_command_pool()
{
	QueueFamilyIndices queue_family_indices = GraphicsBufferManager<GraphicsEngineT>::get_graphics_engine().findQueueFamilies(GraphicsBufferManager<GraphicsEngineT>::get_graphics_engine().get_physical_device());
	...
	if (vkCreateCommandPool(GraphicsBufferManager<GraphicsEngineT>::get_logical_device(), &command_pool_create_info, nullptr, &command_pool) != VK_SUCCESS)
	{
...
VkCommandBuffer GraphicsResourceManager<GraphicsEngineT>::create_command_buffer()
{
	...
	if (vkAllocateCommandBuffers(GraphicsBufferManager<GraphicsEngineT>::get_logical_device(), &allocation_info, &command_buffer) != VK_SUCCESS)
	{
